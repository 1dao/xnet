...
// ===================== 内置 CA 证书数据（抽离到独立文件，方便维护） =====================
// 注意：替换为你完整的 cacert.pem 内容（保留所有 BEGIN/END CERTIFICATE 块和 \n 转义）
//"##\n"
...
//"\n"
//"\n"
static const unsigned char _cacert_pem[] =
"-----BEGIN CERTIFICATE-----\n"
...
// 计算证书数据长度（自动计算，无需手动修改）
static const size_t _cacert_pem_len = sizeof(_cacert_pem);

// ===================== static inline 接口：暴露内置证书（高效无调用开销） =====================
/**
 * @brief 获取内置 CA 证书数据指针（static inline，编译时内联展开）
 * @return 证书数据指针（const unsigned char*，只读）
 */
static inline const unsigned char* httpc_cacert_get_data(void)
{
    // 直接返回证书数组指针，编译器可内联展开，无函数调用开销
    return _cacert_pem;
}

/**
 * @brief 获取内置 CA 证书数据长度（static inline，编译时内联展开）
 * @return 证书数据长度（size_t）
 */
static inline size_t httpc_cacert_get_len(void)
{
    // 直接返回证书长度，编译器可内联展开，无函数调用开销
    return _cacert_pem_len;
}

#endif // HTTPC_CACERT_H